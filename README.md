...
**移动端优先**的零依赖单页应用：无框架、无构建、无后端，只有一个 HTML + 两个 JS + 一个 JSON 数据。
手机上是卡片流 + 底部筛选抽屉，桌面宽度自动变多列；顶栏/搜索框吸顶，支持 iPhone 安全区。
...
- **结果概览**：命中条数、中位单价、中位总价、中位面积、总价区间、日期范围（横滑 chip）
...
| `core.js` | **纯逻辑**：筛选、排序、聚合、CSV 生成；不碰 DOM，可直接在 Node 里测试 |
